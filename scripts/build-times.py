    ...
    for _ in range(5):
        ...
        try:
            subprocess.check_output(
                [f"./orbit build --pacname=orbitgen --mode=development"], shell=True
            )
        except:
...
def setup_repo(dir: str, tag: str) -> str:
    try:
        if tag == "latest":
            subprocess.check_output(
                [f"git clone http://github.com/GuyARoss/orbit"], shell=True
            )
            os.chdir(dir + "/orbit")
        elif "commit@" in tag:
            ...
            subprocess.check_output(
                ["git clone http://github.com/GuyARoss/orbit"], shell=True
            )
            os.chdir(dir + "/orbit")
            subprocess.check_output([f"git checkout {commit}"], shell=True)
        else:
            subprocess.check_output(
                [f"git clone --branch {tag} http://github.com/GuyARoss/orbit"],
                shell=True,
            )
            os.chdir(dir + "/orbit")
